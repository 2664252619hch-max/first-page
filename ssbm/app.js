...
const primaryCategories = [
    ...
    { id: '4', name: '无形资产', icon: '💡' },
    { id: '5', name: '不动产', icon: '🏠' },
    { id: '6', name: '不征税项目', icon: '📋' }
];

// 判断商品属于哪个一级分类
function getPrimaryCategory(code) {
    if (!code) return '6';
...
function renderCategories() {
    ...
    taxCodeData.forEach(item => {
        const catId = getPrimaryCategory(item.code);
        categoryCounts[catId]++;
    });
    
    let html = primaryCategories.map(cat => `
        <div class="category-item" data-category="${cat.id}" data-category-name="${cat.name}">
            <span class="icon">${cat.icon}</span>
            <span class="name">${cat.name}</span>
            <span class="count">${categoryCounts[cat.id]}</span>
        </div>
    `).join('');
...
// 判断输入是否为编码（数字较多）
function isLikelyCode(query) {
    const digitCount = (query.match(/\d/g) || []).length;
    return digitCount >= 3 || /^\d+$/.test(query);
}

// 执行搜索
function performSearch(query) {
    const lowerQuery = query.toLowerCase().trim();
    const isCodeSearch = isLikelyCode(query);
    ...
    const scoredResults = results.map(item => {
        ...
        const exactCodeMatch = item.code === query;
        const startsWithCode = item.code.startsWith(query);
        const includesCode = item.code.includes(query);

        if (isCodeSearch) {
            // 编码搜索权重
            if (exactCodeMatch) score += 1000; // 完全匹配编码
            else if (startsWithCode) score += 500; // 编码前缀匹配
            else if (includesCode) score += 300; // 编码包含匹配
            // 名称匹配作为辅助
            if (nameLower === lowerQuery) score += 200;
            ...
            if (exactCodeMatch) score += 90;
            else if (includesCode) score += 40;
...
function showSuggestions(query) {
    ...
    if (isCodeSearch) {
        // 编码搜索：优先编码匹配
        suggestions = taxCodeData.filter(item => {
            return item.code.includes(query) || 
                   item.code.startsWith(query);
        }).sort((a, b) => {
            // 完全匹配或前缀匹配优先
            const aExact = a.code === query ? 0 : a.code.startsWith(query) ? 1 : 2;
            const bExact = b.code === query ? 0 : b.code.startsWith(query) ? 1 : 2;
...
function handleCategoryClick(e) {
    ...
    // 筛选该一级分类下的商品
    const results = taxCodeData.filter(item => getPrimaryCategory(item.code) === categoryId);
